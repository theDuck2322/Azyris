...
namespace Az
{
	...
	void CP_BoxCollider2D::CreateCollider(Az::Rect& dst, CP_ColliderType type)
	{
		AZ_Assert(m_Space != nullptr, "Scene is nullptr");

		CreateCollider(dst.Position, dst.Size, type);
	}
	void CP_BoxCollider2D::CreateCollider(const glm::vec3& position, const glm::vec3& size, CP_ColliderType type)
	{
		AZ_Assert(m_Space != nullptr, "Scene is nullptr");
		m_ColliderType = type;

		if (type == Az::CP_ColliderType::STATIC)
		{
		...
		else if (type == Az::CP_ColliderType::DYNAMIC)
		{
	...
	{
		AZ_Assert(m_Space != nullptr, "Scene is nullptr");
		AZ_Assert(m_ColliderType != Az::CP_ColliderType::STATIC, "Collider is STATIC!!");
